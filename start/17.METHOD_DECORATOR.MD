...
Not exactly — the prototype is **not literally an array of descriptors**, but it **behaves like a container of properties**, each of which has a **descriptor**. Let me clarify step by step.
...
* `Calculator.prototype` is just an **object**.
* It has keys corresponding to **methods or properties** of the class.
* For example:

```js
console.log(Calculator.prototype);
// Output: { add: [Function: add], constructor: [Function: Calculator] }
```

---

## 2. Each property has a descriptor

You can think of the prototype as **a collection of properties**, and each property internally has a **descriptor**:
...
* `writable` → can the function be replaced
* `enumerable` → shows up in loops
* `configurable` → can we delete or modify the property

✅ So a **prototype is an object** with properties, not an array. Each property has a descriptor.

---

## 3. Prototype vs Array

* Prototype **looks like a normal object** `{ add: function, constructor: function }`
* You **can’t iterate over descriptors like an array** automatically.
* If you want **all descriptors**, use:

```ts
const descriptors = Object.getOwnPropertyDescriptors(Calculator.prototype);
...
```js
{
  constructor: { value: [Function: Calculator], ... },
  add: { value: [Function: add], writable: true, enumerable: false, configurable: true }
}
```

* Now it’s an **object of descriptors**, not an array.

---

### ✅ Key takeaway

* Prototype = object that holds methods/properties.
* Each method/property has a **descriptor** (value, writable, enumerable, configurable).
* Decorators modify the **descriptor.value** (the method itself) rather than the prototype object directly.

---

Ah! Now we’re getting to the **core of how decorators actually override methods internally**. Let’s break it down carefully with a **custom example** and an internal flow.

---

## 1. The key idea

* **Prototype holds the method**: `Calculator.prototype.add = function(a,b){...}`
* Every property on an object (including methods) has a **descriptor**, which has a `value` pointing to the function.
* **Decorators don’t replace the prototype object**, they **replace the descriptor’s value**.
* **At runtime**, when JS does `instance.add()`, it looks up the prototype and calls **whatever function is in the descriptor’s `value`**.
...
```
Instance -> Prototype -> Property Descriptor -> value (Function)
```

* JS looks at `instance.add` → goes to prototype → finds property `add` → reads `descriptor.value` → calls that function.

If a decorator replaces `descriptor.value`, **the prototype property now points to the new wrapper function**, so JS calls that **wrapper function** instead.

---

## 3. Custom example

Let’s simulate it **manually**, without TypeScript decorators:
...
function logMethod(target, key, descriptor) {
  ...
  descriptor.value = function (...args) {
    console.log("Before calling", key, "with args:", args);
    const result = original.apply(this, args);
    console.log("After calling", key, "result:", result);
    return result;
  };
...
// Step 2: Get descriptor manually
let desc = Object.getOwnPropertyDescriptor(Calculator.prototype, "add");
console.log("Original descriptor value:", desc.value);

// Step 3: Apply our decorator manually
desc = logMethod(Calculator.prototype, "add", desc);

// Step 4: Replace descriptor on prototype
Object.defineProperty(Calculator.prototype, "add", desc);

// Step 5: Test
const calc = new Calculator();
console.log("Final result:", calc.add(2, 3));
```
...
1. `Object.getOwnPropertyDescriptor` fetches `{ value: [Function: add], writable: true ... }`.
2. Decorator wraps `value` with a **new function**.
3. `Object.defineProperty` replaces the old `add` on the prototype with the **new descriptor** (new `value` function).
4. When you call `calc.add(2,3)`:

```
calc → no own property 'add' → check prototype
Calculator.prototype.add → descriptor.value → new wrapper function
```

5. Inside wrapper function, `original.apply(this, args)` calls the **original method**.
...
calc.add(2,3)
   |
   v
Prototype lookup -> Calculator.prototype.add
   |
   v
Descriptor.value (wrapper function)
   |
   v
Inside wrapper -> call original.apply(this, args)
   |
   v
...
* JS always calls **whatever function is currently in the prototype’s property**.
* Decorators just **replace the descriptor’s `value`**; the prototype property still exists.
* The original function is kept separately (in `original`) so the wrapper can call it.
